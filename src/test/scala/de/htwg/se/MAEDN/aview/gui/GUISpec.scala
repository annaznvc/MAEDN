...
import de.htwg.se.MAEDN.model.{IState, IManager, IPlayer, IBoard}
...
class GUISpec
  ...
  private var mockController: IController = _
  private var mockManager: IManager = _
  ...
  override def beforeEach(): Unit = {
    mockController = mock[IController]
    mockManager = mock[IManager]
    mockActionManager = mock[ActionManager]

    when(mockController.manager).thenReturn(mockManager)
    when(mockManager.state).thenReturn(State.Menu) // <- KORREKT!
  ...
  "A GUI" should {
    ...
    "initialization" should {
      ...
      "create with proper controller reference" in {
        // gui.controller shouldBe mockController // ENTFERNEN, da private
    ...
    "scene management" should {
      ...
      "switch to menu scene" in {
        runOnFXThread {
          val menuKey = "Menu"
          gui.switchToScene(menuKey)
          gui.sceneCache should contain key menuKey
      ...
      "switch to config scene" in {
        runOnFXThread {
          ...
          gui.switchToScene(configKey)
          gui.sceneCache should contain key configKey
          gui.currentSceneContent should not be null
        }
      }
      ...
      "switch to running scene" in {
        runOnFXThread {
          ...
          gui.switchToScene(runningKey)
          gui.sceneCache should contain key runningKey
          gui.currentSceneContent should not be null
        }
      }
      ...
      "create scene content for each state" in {
        runOnFXThread {
          val menuContent = gui.createSceneContent("Menu")
          val configContent = gui.createSceneContent("Config")
          val runningContent = gui.createSceneContent("Running")
